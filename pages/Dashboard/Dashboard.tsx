...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (currentView === 'statistics' || currentView === 'overview') {
      ...
      setAgents(MockService.getAgents());
    } else if (currentView === 'transaction_management') {
      setTransactions(MockService.getTransactions());
      ...
      case 'overview':
        return (
          <div className="space-y-8">
            ...
            {/* KPIs Grid (extracted) */}
            <DashboardTopStats
              totalDeposits={MockService.getTotalDeposits()}
              usersCount={users.length}
              groupsCount={groups.length}
              adminDepositsAmount={stats?.totalAdminDepositsAmount ?? 0}
              pendingCount={pendingSubmissionsCount}
            />

            {/* Recent Activity Split */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Recent Transactions (extracted) */}
              <RecentTransactionsPanel transactions={transactions} limit={5} onVoirTout={() => handleNavClick('transaction_management')} />
